...
#define NUM_CHARS 256	// 2^8 = 256, 한 바이트
...
Artist* cread_artist_instance(char* name)
{
...
{
	Artist* ptr_artist = cread_artist_instance(name);
...
void add_song(char* artist, char* title, char* path)
{
	...
	Artist *ptr_artist = find_artist(artist);	// 파일이 복잡해질 때는 포인터 변수의 경우 포인터라는 것을 표시해주는 이름을 사용하면 좋음
	if (ptr_artist == NULL) {
...
Artist* find_artist(char* name)
{
	...
	/* 11001111 -> 111...11111001111*/
	while (p != NULL && strcmp(p->name, name) < 0)	// 더 큰 이름이 나온다면 없는 것이기 때문에 끝
		p = p->next;
...
void remove(int index)
{
	...
	if (q == NULL) {	// remove first
		index_directory[index % SIZE_INDEX_TABLE] = p->next;	// p= p->next 실수
	}
	else {				// remove after q
		q->next = p->next;
	}
